...
export function NetworkFocusPill({ focusNode, onStepBack, onClearFocus }: Props) {
  return (
    <span className="inline-flex max-w-full items-center gap-1 rounded-full border border-primary-500/30 bg-primary-600/10 py-1 pl-1.5 pr-2 text-xs">
      <button
        type="button"
        onClick={onStepBack}
      ...
      <span className="min-w-0 truncate font-medium">{focusNode.label}</span>
      <button
        type="button"
        onClick={onClearFocus}
        aria-label="Clear focus"
        title="Show all nodes"
        className="ml-0.5 inline-flex size-4 shrink-0 items-center justify-center rounded-full text-muted-foreground hover:text-foreground"
      >
