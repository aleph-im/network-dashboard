...
export const CHANGELOG: VersionEntry[] = [
  {
    ...
    changes: [
      {
        type: "feature",
        text: "Earnings tab gets a **Reward address breakdown** Card alongside the chart (chart 2/3, breakdown 1/3 on `lg+`, stacked below) on both CRN and CCN node detail views. Horizontal stacked bar splits the reward address's window earnings into four buckets — this node / other same-kind nodes / cross-kind ops / staking — so operators using one reward address across many nodes can see this node's contribution in portfolio context. Bidirectional hover: hovering a segment or legend row dims the rest. When the reward address only earned from this node in the window, a one-liner caption replaces the bar. `View full wallet →` deep links to `/wallet?address=`. No new API calls — built on the existing `summary.recipients`. The tab loading gate now also honours `isPlaceholderData`, so switching range (24h → 7d → 30d) shows the skeleton during refetch instead of leaving stale numbers on screen.",
      },
    ],
  },
  {
