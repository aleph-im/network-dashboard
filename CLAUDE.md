...
- Sidebar accordion structure (`src/config/nav.ts`, `NAV_SECTIONS`): four sections — Dashboard (Overview), Resources (Nodes, VMs, Credits), Network (Graph, Health), Operations (Issues). Each section is an independently collapsible DS `AccordionSection`; per-section open/closed state persists in `localStorage["sidebar.section.<id>"]` via DS `useAccordionState`. Dashboard / Resources / Network default to open; **Operations defaults to closed** (`defaultOpen={false}`) so the diagnostic surface stays a deliberate click rather than always-on noise. The old "More" overflow popover (`UtilityMenu`) is gone — Issues and Network Health are top-level entries inside Operations and Network respectively, no longer hidden behind a click.
