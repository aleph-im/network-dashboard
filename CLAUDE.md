...
- `PageHeader` slot (DS) renders a sticky chrome row above page content: `leading` slot for the ☰ toggle (visible only when the sidebar is collapsed or below `md`), title in the centre, action slot on the right. Driven by `PageHeaderProvider` + the `usePageHeader` hook (registered for the app in `src/app/providers.tsx` inside `WebSocketProvider`). Pages will call `usePageHeader` to register custom titles + Refresh actions in the next PR; today the shell falls back to `fallbackTitle={routeTitle(pathname)}`, where `routeTitle` (`src/lib/route-title.ts`) maps `/` → `Overview`, `/nodes` → `Nodes`, `/vms` → `VMs`, `/credits` → `Credit Expenses`, `/network` → `Network Graph`, `/status` → `Network Health`, etc.
