...
- Shared filter chrome: `FilterToolbar` (optional `leading` slot, status pills, icon-only filter toggle with active badge dot, search input with clear) and `FilterPanel` (collapsible glassmorphism card with reset button) used by list pages and Issues page
