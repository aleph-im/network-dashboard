...
- Responsive layout: full-screen drop-down menu below `md` (via `MobileMenu` rendering `NAV_SECTIONS` + cross-product footer band; menu state from `useMobileMenu` in `src/hooks/use-mobile-menu.ts`, auto-closes on route change, on `md+` resize, and on `Escape`; body scroll locked while open); inline expanded ↔ icon-rail sidebar above `md` (via DS `useSidebarCollapse`). On mobile, the hamburger sits in the top-right and is replaced by the menu's own × button when open; `ProductStrip` is hidden and lives inside the menu footer. Menu open/close animate via a `closed → open → closing → closed` phase machine — slide-in is `translateY(-12px) + opacity(0→1)` over 320ms `--ease-spring`, slide-out is the reverse over 220ms ease-in; `onAnimationEnd` on the panel unmounts after the exit animation finishes (or the effect skips straight to `closed` when `prefers-reduced-motion: reduce`). Header and footer dividers were dropped — the panel reads as one continuous surface and the footer's `bg-muted/40 dark:bg-surface` tint anchors the bottom band without a hard rule. Per-page Refresh actions are desktop-only (auto-polling at 15–30s covers freshness on mobile); the Wallet page keeps an `Open in Explorer →` link inline on mobile. Detail panels as slide-in overlays on mobile, inline on desktop; adaptive column hiding when detail panel is open. Wide tables (Wallet Nodes/VMs, Credit Recipients, Earnings per-VM) render as stacked cards via `MobileTableCardRow` below `md`; each card field can opt into a stacked layout (label above value at full width) via `stack: true` on its `MobileCardField`, which the Credits Recipients Sources field uses so wrapped chips don't crowd against the label. Credits flow diagram swaps to `CreditFlowList` below `md`. NodeEarningsChart tooltip is inline-below-chart on mobile, floating side-anchored on desktop.
- Shared `ProductStrip` (DS, from `@aleph-front/ds@0.20.0+`) anchors the top of every page: Aleph logomark linking to `https://aleph.cloud`, four product tabs (Cloud · Network · Explorer · Swap) with `activeId="network"` so the dashboard reads as part of the Aleph product family, theme toggle docked in the right slot (moved out of the old `AppHeader`). Tab list driven by `src/config/apps.ts` (`APPS` + `ACTIVE_APP_ID`); the three cross-app entries (Cloud, Explorer, Swap) are flagged `external: true` so DS renders an external-link affordance on those tabs — Network stays internal since it's the current app. Fixed 54px height (DS-owned cross-product anchor); the DS default `border-b` is suppressed via `className="border-b-0"` so the chrome flows continuously into the rounded surface below (Decision #95).
- `AppShellSidebar` (DS) replaces the old `AppSidebar`: collapsible between expanded ↔ icon-rail via a built-in toggle anchored at the sidebar's bottom; state persists in `localStorage["sidebar.collapsed"]` via the DS `useSidebarCollapse` hook. `AppMark` (`src/components/app-mark.tsx`) sits in the brand slot — just the "Network" wordmark, no logomark (the cross-product Aleph mark lives in `ProductStrip` so duplicating it in the sidebar would read as two anchors competing for the same role); `AppMark` returns `null` when collapsed so the rail stays clean. The version link in the sidebar bottom row uses the DS `footer` prop (added in `@aleph-front/ds@0.19.0`); it auto-hides and the DS chevron re-centers when the sidebar is collapsed — no manual rail-hide handling. The rounded `bg-background` surface (the recessed content panel + accent radial glow) now starts directly under `ProductStrip` and **envelopes** the `PageHeader` so chrome flows continuously: ProductStrip → rounded surface → PageHeader → main. Scroll-to-top on route change is still owned by `app-shell.tsx`.
- Sidebar accordion structure (`src/config/nav.ts`, `NAV_SECTIONS`): four sections — Dashboard (Overview), Resources (Nodes, VMs, Credits), Network (Graph), Operations (Health, Issues). Each section is an independently collapsible DS `AccordionSection`; per-section open/closed state persists in `localStorage["sidebar.section.<id>"]` via DS `useAccordionState`. Dashboard / Resources / Network default to open; **Operations defaults to closed** (`defaultOpen={false}`) so the diagnostic surface stays a deliberate click rather than always-on noise. The old "More" overflow popover (`UtilityMenu`) is gone — Network Health and Issues are top-level entries inside Operations, grouped together because both are operational-diagnostic surfaces (the Network section is reserved for topology views).
- `PageHeader` slot (DS) renders a sticky chrome row above page content: `leading` slot for the ☰ toggle (visible only when the sidebar is collapsed or below `md`), title in the centre, action slot on the right. The DS default `bg-background/95` is overridden via `className="bg-transparent ..."` so the `main-glow` accent radial gradient bleeds through behind the title row; the DS `border-b` is kept so the row still reads as a discrete band. Title styling is also tuned down via a Tailwind arbitrary variant on the same `className` (`[&_.truncate]:text-xs [&_.truncate]:text-muted-foreground`) so the title sits as quiet metadata rather than competing with the page's `<h1>`. Driven by `PageHeaderProvider` + the `usePageHeader` hook (registered for the app in `src/app/providers.tsx` inside `WebSocketProvider`). Every page now calls `usePageHeader` to register its own title (and, where useful, its actions): Overview / Credit Expenses / Network Graph / Network Health / Issues / Changelog ship static titles; Nodes (`Nodes · N total`) and VMs (`VMs · N total`) become data-aware once their fleet loads; Wallet shows the truncated address (`0x1234…5678`) or `Wallet` when none is provided. Refresh actions on Nodes / VMs / Network Health / Issues / Wallet render as quiet `<Button variant="text" size="xs" iconLeft={<ArrowClockwise />}>` controls — disabled with `Refreshing…` label while in flight. They render only above `md` (`hidden md:inline-flex`) in the PageHeader right slot; on mobile the Refresh button is dropped entirely because React Query auto-polls at 15–30s and the inline mobile row added in 0.28.0 was redundant noise. The icon goes through the DS `iconLeft` slot (not as a child) because the DS Button wraps children in a single inline `<span>`; passing icon + text as siblings stacks them vertically. Network Health's body-level Recheck button was removed; Wallet adds an `Open in Explorer →` link beside Refresh. Pages that don't call the hook still fall back to `fallbackTitle={routeTitle(pathname)}` where `routeTitle` (`src/lib/route-title.ts`) maps `/` → `Overview`, `/nodes` → `Nodes`, etc.
- NavItem wiring: DS `NavItem asChild` clones a Next.js `<Link>` for SPA navigation (no full-page reloads); `/credits` keeps its hover-prefetch by passing `onMouseEnter` / `onFocus` through the `asChild` cloneElement to the underlying anchor (DS Decision #80 added `AnchorHTMLAttributes` passthrough in `@aleph-front/ds@0.18.0` so this works without leaking dashboard-specific wiring into the DS).
