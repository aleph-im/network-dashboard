...
- Network Health page (`/status`): left-aligned title with status `Badge` (success/error), glassmorphism stat cards (endpoints healthy, avg latency, last checked + recheck button), Scheduler API + Aleph API endpoint sections side-by-side (`lg:grid-cols-2`) with StatusDot/HTTP code/latency, auto-refresh every 60s, `?api=` URL override. The Scheduler API list leads with a "WebSocket stream" row sourced from `useWebSocketStatus()` — green/healthy when `connected`, amber/degraded while `connecting…` or `reconnecting…`, grey/offline when `disconnected`; right column shows `connected · awaiting events` until the first event, then `N events · last <relative time>` (`Ns/m/h ago`).
