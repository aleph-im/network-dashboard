...
- Worldmap card on Overview hero (`WorldMapCard`): equirectangular world map (Wikimedia BlankMap-Equirectangular, public domain SVG in `public/world-map.svg`) with one green SVG `<circle>` per active node (CRN+CCN combined into one color), positioned at country centroid + deterministic per-hash scatter (~1.5°). Live node state from `useNodeState()` joined with a build-time JSON snapshot (`src/data/node-locations.json`) keyed by node hash; snapshot generated by `scripts/build-node-locations.ts` (CCN multiaddr IP parsing + CRN hostname DNS resolution + `ip3country` country lookup) chained into `pnpm build` as a prebuild step. Hash-seeded per-dot flicker animation (4–6s, 0–5s delay) respecting `prefers-reduced-motion`. Header shows a green status dot, "Aleph Cloud Nodes" label, and a disabled expand button with "Coming soon" tooltip (interaction deferred to v2). Snapshot script aborts and keeps the previous JSON if api2 is unreachable or returns < 50% of the previous dataset, so production builds never silently regress to an empty map. Wikimedia attribution link bottom-left.
