...
- Shared filter chrome: `FilterToolbar` (optional `leading` slot, DS Tabs pill variant with `overflow="collapse"` for status filters with sliding indicator and automatic overflow dropdown, icon-only filter toggle with active badge dot, search input with clear) and `FilterPanel` (collapsible glassmorphism card with reset button) used by list pages and Issues page; toolbar always renders above the table+detail flex container so it never gets squeezed by the detail panel
