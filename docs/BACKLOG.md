...
- ✅ 2026-05-11 - Network graph geo layer — fifth `geo` toggle on `/network` (off by default) that emits one `kind: "country"` node per represented country (pinned at projected centroid via `networkMercator`, `fx`/`fy` in both warmup and live sim) plus one geo edge per located CCN/CRN to its country; geo edges live in a separate `forceLink` (distance 40, strength 0.6) so the relational `forceLink` keeps d3's degree-aware default strength; geo edges never render (force only — proximity says it), country labels bypass `LABEL_ZOOM_THRESHOLD`; countries are clickable/focusable/searchable, search routes country matches through the focus action (`?focus + ?selected`); detail panel adds `NetworkDetailPanelCountry` with flag emoji + CCN/CRN/owner stat tiles aggregated from incident geo edges; legend gains a country row when geo is on. Stakers/rewards/unlocated nodes float as today; no world-map backdrop (Decision #74, plan `docs/plans/2026-05-11-network-map-geo-layout-plan.md`)
