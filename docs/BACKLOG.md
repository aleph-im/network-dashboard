...
### 2026-06-11 - ② Node Earnings tab: bounded execution-only per-bucket weights (retargeted)
**Source:** Originally "Owner view: fetch execution-only expenses for per-node weights" (2026-06-09). **Obsolete for the owner view** — Decision #112 dropped the expense feed there entirely and weights the per-node split by live `vmCount` from `useNodes()`, so the owner-view by-node breakdown no longer fetches expenses at all. The underlying problem **resurfaces in ② Node Earnings tab** (Plan B), which needs *per-bucket, per-node* historical weights that a single-point `vmCount` can't provide.
**Description:** When ② re-sources the per-node ALEPH-over-time chart, it must NOT reuse `useCreditExpenses(fullWindow)` — that pulls the whole-network credit-expense feed (measured: 24h ≈ 112MB, 10d ≈ 750MB; `getCreditExpenses` has no timeout, so it hangs — Decision #112). Fetch only `type_execution` messages over a **bounded** window, add a request timeout, and consider a server-side aggregate. Investigate whether api2 `messages.json` can filter by content tag (storage carries no `node_id` and is unused for per-node weights). Gates ② chart performance.
**Priority:** Medium
...
**Description:** Re-source the `/nodes?view=<hash>&tab=earnings` KPI, by-source, and the per-node ALEPH-over-time chart from the new rewards layer (`useRewards` + `apportionOwnerRewards`), including each node's wage slice. The chart is the heaviest piece: per-node per-bucket values are apportioned from the API's per-address per-bucket totals via per-bucket api2 weights. Add an inline by-source bar under the "ALEPH accrued" KPI (treatment A). Spec section "② Node Earnings tab" in `docs/superpowers/specs/2026-06-09-node-owner-revenue-view-design.md`. ⚠️ Those per-bucket api2 weights must come from a **bounded, execution-only, timeout-protected** fetch — naively reusing `useCreditExpenses(fullWindow)` downloads the whole-network feed (~750MB for a 10-day cycle) and hangs, the exact failure the owner view hit (Decision #112). See the retargeted "bounded execution-only per-bucket weights" item above. The owner view's single-point `vmCount` shortcut does NOT work here — the chart needs per-bucket history.
**Priority:** High
