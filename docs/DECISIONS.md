...
**Decision:** Add a `WorldMapCard` to the Overview hero alongside a slimmed 2×2 stat grid (Nodes Total/Healthy + VMs Total/Dispatched). All non-removed nodes (`inactiveSince == null`), CRN+CCN combined into a single dot color. No interaction in v1 — disabled expand button with "Coming soon" tooltip; per-dot flicker is a hash-seeded subtle pulse with `prefers-reduced-motion` opt-out. Geolocation comes from a build-time JSON snapshot generated by `scripts/build-node-locations.ts` (CCN multiaddr IP parsing + CRN hostname DNS resolution + `ip3country` lookup), keyed by node hash. Country centroids come from a one-shot snapshot of the `world-countries` package. Dropped from the hero: Unreachable/Removed/Missing/Unschedulable cards (still reachable as per-status pills on `/nodes` and `/vms`).
**Rationale:** Both-types-one-color matches the design intent (fleet visibility, not breakdown) and keeps the legend off the card. All-non-removed is the right denominator for "where the network lives" — including unreachable nodes shows infrastructure that exists even when the scheduler can't reach it. Build-time pre-resolution is the only sane path: runtime DNS would require DoH (per-client cost, network noise, ad-blocker noise), and IP-to-country needs a DB anyway — bundling once at build time collapses both to a static JSON read. The 50% abort guard on the snapshot script prevents api2 outages from silently shipping an empty map. Tight scatter (1.5°) around country centroids gives visual density without geocoding to city level (which would need a much larger DB and rotating per-city data). Hash-seeded per-dot animation timing avoids the "single synchronized blink" aesthetic without requiring per-frame coordination. Slimming the stat grid to 4 cards is a forced consequence of giving the map space — the dropped cards are operational metrics, not scale metrics, and they're one click away on the list pages.
**Alternatives considered:** Runtime DoH for live geolocation (rejected — cost, fragility, no aesthetic gain over a daily snapshot). Per-city dots via a larger geo DB (rejected — bigger bundle, rotating data, no real story for the v1 audience). Color-coded CRN vs CCN dots (rejected — adds legend complexity, splits attention; deferred to v2). Click-to-detail / hover popover (rejected for v1 — needs tooltip surface and per-dot picker logic; deferred). Keeping all 8 stat cards plus map (rejected — hero overflows; the dropped cards are list-page filters, not scale signals). D3.js / react-simple-maps (rejected — pure SVG `<circle>` over an inline equirectangular `<Image>` is ~30 lines of code; bringing in D3 is overkill for v1).
