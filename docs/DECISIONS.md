...
**Decision:** (1) Extend the search predicate to also match any node name where `node.reward || node.owner === r.address`, by building an `address → [{ name, kind }]` lookup once per `nodeState` change with `useMemo`. Placeholder updated to "Search address or node name…". (2) When a row matched only via node name (not address), render a small `Badge fill="outline" variant="info" size="sm"` in the Sources cell reading `Matched: <name>` (truncated to 12ch, with `+N` overflow and a `title` tooltip listing all matched names). (3) Make the whole row clickable via DS Table's `onRowClick`, navigating to `/wallet?address=…`. The `CopyableText` cell already calls `e.stopPropagation()` on its copy button and address link, so clicking copy still copies without triggering the row navigation.
