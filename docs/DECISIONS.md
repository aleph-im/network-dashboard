...
**Alternatives considered:** Bundling VM-status long-tail (rejected — confusing). Toolbar placement next to status pills (rejected — toolbar is for navigation/scope, not advanced filters). Inline banner on culling state (rejected — too loud for a default-on filter). Always showing `filtered/unfiltered` (rejected — noisy by default). Doing the tab cap and the filter together in one PR (rejected — DS `maxVisible` prop wasn't ready, splitting kept the operationally meaningful change unblocked).
