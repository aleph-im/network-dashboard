...
**Focus mode** (`?focus=<id>`) replaces `visibleGraph` with `egoSubgraph(fullGraph, focusId)` — the focused node, its 1-hop neighbors, and the edges between them. Focus is layered: clicking a neighbor's "Focus" action drills deeper, replacing the focus target. Focus state is exposed as a compact pill in the toolbar row (`Focused: <name> ×`) — always visible regardless of selection state, and × clears focus to return to the full graph. The earlier full-width focus banner was dropped because the panel covered it.
