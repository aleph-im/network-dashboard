...
- Charge `-180`, link distance `60` strength `0.5`, center force at `(size.w/2, size.h/2)` updated separately on resize without restarting the simulation (so opening the detail panel — which resizes the viewport — doesn't re-shake the graph).
